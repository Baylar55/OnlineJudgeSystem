...
public class CompileProblemCommandHandler : IRequestHandler<CompileProblemCommand, CompileProblemCommandResponse>
{
    public async Task<CompileProblemCommandResponse> Handle(CompileProblemCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var syntaxTree = SyntaxFactory.ParseSyntaxTree(code, cancellationToken: cancellationToken);
            ...
            if (!emitResult.Success)
            {
                ...
                return new CompileProblemCommandResponse("Compilation Error: " + string.Join(", ", errors), 0, 0, SubmissionStatus.CompilationError);
            }
            else
            {
                ...
                return new CompileProblemCommandResponse(testResults, elapsedMilliseconds, (long)memoryUsageMB,
                testResults.Contains("error") ? SubmissionStatus.CompilationError : SubmissionStatus.Accepted);
            }
        }
        catch (Exception ex)
        {
            return new CompileProblemCommandResponse($"Exception caught: {ex.Message}", 0, 0, SubmissionStatus.CompilationError);
        }
    }

    private static string ExecuteTestCases(Assembly assembly, List<TestCase> testCases, string methodName)
    {
